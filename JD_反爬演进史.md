...
| 10 | 2026-06-19 | （承接 9）账号级封控下,批间冷却空等占了大头(570 条约 3.7h) | **账号交替**:`rotate_profile()` 每批切到下一个已登录账号,当前账号歇"对方那一批"(~21min ≫ 安全 10min),免空等;~12h→8h（待 2 账号实测） |
...
## Phase 10 — 账号交替:用"对方批次"填掉冷却空窗（2026-06-19）

Phase 9.2 确认了频控是账号级,这一步是它的解法。

**关键认知**:
- **profile 与账号 1:1 绑定**——一个 profile 就是某账号扫码后的持久登录态容器,"切 profile = 切账号",运行时不重新登录,只切到另一个已登录好的 profile。N 个账号 = N 个 profile。
- 常规档(25/批 + 歇 10min)单账号本就能稳跑几百条(Phase 9 那次 370 条就是单账号跑完的)。真正的瓶颈不是"账号额度",是**批间冷却空等**(570 条约 3.7h 全在等)。

**机制**:`crawler.rotate_profile()`(带 wrap 的轮换),**每批之间切到下一个已登录账号**继续跑。当前账号自然休息"对方那一批"的时长(~21min ≫ 已验证安全的 10min)→ 既到不了惩罚态,又免去空等。批间三分支:
1. 账号池耗尽(`current_profile_id is None`,全员被封)→ 冷却,给账号自愈时间;
2. 轮换成功 → **免冷却**,直接下一批;
3. 只剩 1 个可用账号 → 退回已验证的 600s 冷却(`single_account_mode` 标志,避免对登录过期 profile 反复 close/launch)。

**效果**:570 条 / 2 账号 / 常规档 ≈ 12h → 8h,每账号负载减半。**`rotate_profile` 分支/wrap/恢复逻辑已离线 mock 验证;端到端(交替真正跑起来)需先把池灌成 2 个不同账号再实跑——写下本段时用户仅有 2 个账号且其一仍在频控自愈中,尚未实测。** commit `0cd6171`。
